...
pub async fn get(req: AuthedRequest<'_>) -> ServerResult {
    ...
    let actor = queries::get_or_search_for_actor(&handle, &req.locals.current_profile).await?;
    ...
    let outbox = get_outbox(&outbox_uri, &req.locals.current_profile).await?;
    ...
    let page = get_outbox_page(&first_page_url, &req.locals.current_profile).await?;
