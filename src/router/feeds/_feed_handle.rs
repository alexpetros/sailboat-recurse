...
pub async fn get(req: AuthedRequest<'_>) -> ServerResponse {
    ...
    let actor = queries::get_or_search_for_actor(&handle, &req.current_profile().unwrap()).await?;
    ...
    let outbox = get_outbox(&outbox_uri, &req.current_profile().unwrap()).await?;
    ...
    let page = get_outbox_page(&first_page_url, &req.current_profile().unwrap()).await?;
